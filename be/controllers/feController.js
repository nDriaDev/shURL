...
export default function feController(req, res, next) {
	try {
		...
		if (req.originalUrl.startsWith(CONSTANTS.PATHS.API)) {
			...
			["index.html", INITIAL, GENERATE, SIGNIN, SIGNUP].includes(req.originalUrl) ? res.setHeader("X-Robots-Tag", "index, follow") : res.setHeader("X-Robots-Tag", "noindex, nofollow");
